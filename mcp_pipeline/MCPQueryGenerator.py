        ...
        try:
            ...
            try:
                ...
                if "401" in str(e) or "auth" in str(e).lower():
                    ...
                    logger.error("Your API key is not being accepted by OpenRouter.")
                    logger.error("Please verify:")
                    logger.error("1. You have a valid OpenRouter API key (starts with 'sk-or-v1-')")
                    logger.error("2. Your account has credits available")
                    logger.error("3. The API key is correctly set in your .env file")
                    return []
                
                # Try with direct HTTP request as fallback
                logger.info("Attempting direct HTTP request as fallback...")
                try:
                    import requests
                    
                    headers = {
                        "Authorization": f"Bearer {self.api_key}",
                        "HTTP-Referer": "https://github.com/your-project",  # OpenRouter may require this
                        "X-Title": "MCP Query Generator",  # Optional but recommended for OpenRouter
            ...
            try:
                ...
                if start_idx >= 0 and end_idx > start_idx:
                    ...
                    for i, entry in enumerate(generated_queries):
                        ...
                        # Check if it uses multiple servers from the tool_chain
                        tool_chain = entry.get("tool_chain", [])
                        servers_used = set()
                        expected_tools = []
                        
                        for tool_step in tool_chain:
                            if "server" in tool_step:
                                servers_used.add(tool_step["server"])
                            if "tool" in tool_step:
                                expected_tools.append(tool_step["tool"])
                        
                        # If no tool_chain, fall back to expected_tools
                        if not tool_chain and "expected_tools" in entry:
                        ...
                        # Add expected_tools if not present
                        if "expected_tools" not in entry:
                            entry["expected_tools"] = expected_tools
