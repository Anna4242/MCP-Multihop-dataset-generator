...
NOW WITH STEP-BY-STEP REWARD EVALUATION FOR GRPO TRAINING
...
# Import step reward system
from step_reward_evaluator import StepRewardEvaluator, RewardTracker, StepReward
...
class MCPQueryExecutor:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.reward_evaluator = StepRewardEvaluator(
            api_key=self.api_key,
            api_base=OPENAI_API_BASE
        )
        self.reward_tracker = RewardTracker(
            output_dir=Path("reward_trajectories")
        )
        
        # Context tracking for rewards
    ...
    async def _tool_callback(self, tool_call_data):
        """Callback function for tracking tool usage with step rewards."""
        ...
        try:
            # Build task context
            task_context = self.current_query_context
            if self.previous_steps:
                recent_outputs = [
                    str(s.get("output", ""))[:200] 
                    for s in self.previous_steps[-2:]
                ]
                task_context += "\nRecent: " + " | ".join(recent_outputs)
            
            # Get available tools
            available_tools = [t.name for t in self.agent._tools]
            
            # Evaluate step
            step_reward = await self.reward_evaluator.evaluate_step(
            ...
            # Track reward in reward tracker
            self.reward_tracker.add_step(
            ...
            # Log reward info
            logger.info(f"Step {current_step} reward: {step_reward.total:.3f} "
                       f"(relevance={step_reward.tool_relevance:.2f}, "
                       f"progress={step_reward.progress_made:.2f})")
            
        except Exception as e:
            logger.warning(f"Step reward evaluation failed: {e}")
            # Add default reward
            tool_call_data["step_reward"] = {
                "total": 0.5,
        ...
        compatibility_call = {
            ...
            "step_reward": tool_call_data.get("step_reward", {})  # Add reward
        }
    ...
    async def run_query(self, query_data: Dict, idx: int, total: int) -> Dict:
        ...
        self.previous_steps = []
        self.current_query_context = f"Query: {prompt}"
        ...
        # End trajectory
        if rec.get("answer") or rec.get("error"):
    ...
    async def execute_queries(self) -> int:
        ...
        try:
            ...
            # Save rewards
            if hasattr(self, 'reward_tracker') and self.reward_tracker.trajectories:
                # Save to CSV for GRPO training
                csv_path = self.reward_tracker.save_to_csv()
                print(f"💰 Step rewards saved to CSV: {csv_path}")
                
                # Save detailed JSON
                json_path = self.reward_tracker.save_trajectories_json()
                print(f"📊 Detailed trajectories saved to: {json_path}")
                
                # Print reward summary
                total_trajectories = len(self.reward_tracker.trajectories)
                if total_trajectories > 0:
                    avg_reward = sum(t["total_reward"] for t in self.reward_tracker.trajectories) / total_trajectories
                    avg_steps = sum(t["num_steps"] for t in self.reward_tracker.trajectories) / total_trajectories
                    ...
                    if avg_steps > 0:
                        print(f"   Average reward per step: {avg_reward/avg_steps:.3f}")
